...
/**
 * Wizard that automates the creation of an operating system model file.
 *
 */
public class NewOperatingSystemFileWizard extends NewMICOBSFileWizard {
	...
	protected void generateFile(MICOBSResourceCreationPage page,
		...
		MOperatingSystem os = pdlFactory.eINSTANCE.createMOperatingSystem();
		pdlPackageFile.setPackage(creationPage.getSelectedPackage());
		os.setName(creationPage.getModelName());
		os.setVersion(creationPage.getVersion());
		os.getLanguages().addAll(creationPage.getSelectedLanguages());
		pdlPackageFile.setElement(os);

		try {
			PDLGeneratorUtil.generateDefaultMOperatingSystem(creationPage.getRootFolder().getLocation().toPortableString(), 
				pdlPackageFile,
	...
	protected MICOBSResourceCreationPage getResourceCreationPage() {
		if (creationPage == null)
		{
			...
			try {
				...
				creationPage.setTitle(PDLPlugin.INSTANCE.getString("_UI_NewOperatingSystemFileWizardPageTitle"));
				creationPage.setDescription(PDLPlugin.INSTANCE.getString("_UI_NewOperatingSystemFileWizardPageDescription"));
				creationPage.setModelLabel("Operating System:");
				creationPage.setSelectMultipleLanguages(true);
