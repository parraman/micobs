...
/**
 * Wizard that automates the creation of a device model file.
 *
 */
public class NewDeviceFileWizard extends NewMICOBSFileWizard {
	...
	protected void generateFile(MICOBSResourceCreationPage page,
		...
		MDevice device = pdlFactory.eINSTANCE.createMDevice();
		pdlPackageFile.setPackage(creationPage.getSelectedPackage());
		device.setName(creationPage.getModelName());
		device.setVersion(creationPage.getVersion());
		pdlPackageFile.setElement(device);

		try {
			PDLGeneratorUtil.generateDefaultMDevice(creationPage.getRootFolder().getLocation().toPortableString(), 
				pdlPackageFile,
	...
	protected MICOBSResourceCreationPage getResourceCreationPage() {
		if (creationPage == null)
		{
			try {
				...
				creationPage = new MICOBSResourceCreationPage("DeviceFileCreationPage",
						getFileExtension(), getInitialFolder(), null, PDLLibraryManager.getLibraryManager().getAllPackages());
				
				creationPage.setTitle(PDLPlugin.INSTANCE.getString("_UI_NewDeviceFileWizardPageTitle"));
				creationPage.setDescription(PDLPlugin.INSTANCE.getString("_UI_NewDeviceFileWizardPageDescription"));
				creationPage.setModelLabel("Device:");
