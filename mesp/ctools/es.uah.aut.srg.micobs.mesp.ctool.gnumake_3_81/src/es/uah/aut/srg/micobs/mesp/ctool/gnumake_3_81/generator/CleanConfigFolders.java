...
/**
 * Handler of the command that cleans the config folders that
 * are no longer valid.
 *
 */
public class CleanConfigFolders extends GenerateResourcesHandler {

	@Override
	protected String getConfirmDialogMessage() {
		return MESPCToolGNUMake_3_81Plugin.INSTANCE.getString("_UI_CreateConfigFolders_ConfirmDialog_message");
	}
	...
	protected List <IResource> getResourcesToErase(IContainer rootFolder,
		...
		IFolder mainFolder = rootFolder.getFolder(
				new Path(MESPCToolGNUMake_3_81Plugin.INSTANCE.getString("_MESPDeploymentProject_config_foldername")));
		
		MMESPDeployment deployment = (MMESPDeployment) ((MMESPDEPPackageFile)model).getElement();
		
		Set<String> configFolderNames = GNUMakeStringHelper.getConfigFolderNames(deployment);
		...
		if (mainFolder.exists() && mainFolder.members().length != 0)
		{
			...
			for (int i = 0; i < contents.length; i++)
			{
				if (contents[i] instanceof IFolder)
				{
					...
					boolean found = false;
					for (Iterator<String> s = configFolderNames.iterator(); s.hasNext(); )
					{
						String name = s.next();
	...
	protected String getModelClassErrorMessage() {
		return MESPCToolGNUMake_3_81Plugin.INSTANCE.getString("_UI_CreateConfigFolders_ModelClassError_message");
	}
