...
entryRuleMMESPOSSWIPackageFile returns [EObject current=null] 
	:
	{ newCompositeNode(grammarAccess.getMMESPOSSWIPackageFileRule()); }
	 iv_ruleMMESPOSSWIPackageFile=ruleMMESPOSSWIPackageFile 
	 { $current=$iv_ruleMMESPOSSWIPackageFile.current; } 
	 EOF 
;

// Rule MMESPOSSWIPackageFile
ruleMMESPOSSWIPackageFile returns [EObject current=null] 
    @init { enterRule(); 
    }
    @after { leaveRule(); }:
(	otherlv_0='package' 
...
entryRuleMMESPOSSWIPackageElement returns [EObject current=null] 
	:
	{ newCompositeNode(grammarAccess.getMMESPOSSWIPackageElementRule()); }
	 ...
	 { $current=$iv_ruleMMESPOSSWIPackageElement.current; } 
	 EOF 
;

// Rule MMESPOSSWIPackageElement
ruleMMESPOSSWIPackageElement returns [EObject current=null] 
    @init { enterRule(); 
    }
    @after { leaveRule(); }:
    ...
    { 
        $current = $this_MOSSwInterface_0.current; 
        afterParserOrEnumRuleCall();
    }
...
entryRuleMOSSwInterface returns [EObject current=null] 
	@init { 
	...
	{ newCompositeNode(grammarAccess.getMOSSwInterfaceRule()); }
	 ...
	 { $current=$iv_ruleMOSSwInterface.current; } 
	 EOF 
;
finally {
...
// Rule MOSSwInterface
ruleMOSSwInterface returns [EObject current=null] 
    @init { enterRule(); 
		UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
			grammarAccess.getMOSSwInterfaceAccess().getUnorderedGroup_4()
		);
